...
// Request-level cache (dedupe only) AND Data Cache (ISR)
// CACHE STRATEGY:
// 1. request-level: cache() ensures multiple calls in same render pass don't re-fetch
// 2. cross-request: unstable_cache() ensures all users share the same data for 5 minutes
// This solves the "5 table scans per request" critical scalability issue.
export const getTrendingTopicsData = cache(async () => {
    return await getCachedTrendingTopics();
});

// NOTE: We MUST import createClient from @supabase/supabase-js directly here 
// because unstable_cache cannot access cookies(), enabling global shared cache.
import { createClient as createSupabaseClient } from '@supabase/supabase-js';

const getCachedTrendingTopics = unstable_cache(
    async () => {
        // Create a direct client without cookies for global caching
 ...
 */
export const getSimilarTopicsData = cache(async (slug?: string) => {
    return await getCachedSimilarTopics(slug);
});

const getCachedSimilarTopics = unstable_cache(
    async (slug?: string): Promise<SimilarEntity[]> => {
        if (!slug) return [];
 ...
 * OPTIMIZED: Request-level cache + reduced query limit (30 instead of 150)
 */
export const getMatchCenterData = cache(async (limit = 12) => {
    return await getCachedMatchCenterData(limit);
});

const getCachedMatchCenterData = unstable_cache(
    async (limit: number) => {
        // Direct client (no cookies) for ISR
        const supabase = createSupabaseClient(
            process.env.NEXT_PUBLIC_SUPABASE_URL!,
            process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!
        );
